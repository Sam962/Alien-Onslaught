...
if(count < amount) { //if count is less than a specified amount, spawn an enemy and increment count by 1
	instance_create_layer(x, y, "wall_and_barrier", obj_enemy1)
	count++
	alarm[0] = spawnrate;
	if (global.level >= 3 && global.level < 6){
		// Enemy 2 spawns every 8th spawn (waves 3, 4, 5)
		if ((count % 8) == 0){
			instance_create_layer(x, y, "wall_and_barrier", obj_enemy2)
		...
		if ((count % 9) == 0){
			instance_create_layer(x, y, "wall_and_barrier", obj_enemy3)
		}
